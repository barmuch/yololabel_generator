...
export default function TemplateManagementPage() {
  ...
  const refreshProjectCounts = async () => {
    try {
      const response = await fetch('/api/class-sets/refresh-counts', {
        method: 'POST'
      });
      ...
      if (data.success) {
        alert(`✅ Refreshed project counts! Updated ${data.updates.length} templates.`);
        fetchTemplates(); // Reload templates with updated counts
      } else {
        alert(`Error: ${data.error}`);
      }
    } catch (error) {
      console.error('Error refreshing counts:', error);
      alert('Failed to refresh project counts');
    }
  };
